...
class ActorCriticNetwork(nn.Module):
    ...
    def __init__(self, state_size, action_size, fc_layer_sizes, seed):
        ...
        self.std = nn.Parameter(torch.ones(action_size)*0.1)
        ...
        if len(fc_layer_sizes) == 0:
            ...
            for i in range(1, len(fc_layer_sizes)):
                self.linears_act.append(nn.Linear(fc_layer_sizes[i-1], fc_layer_sizes[i]))
        ...
        if len(fc_layer_sizes) == 0:
            ...
            for i in range(1, len(fc_layer_sizes)):
                self.linears_cri.append(nn.Linear(fc_layer_sizes[i-1], fc_layer_sizes[i]))
    ...
    def forward(self, x, action=None, scale=1.0):
        ...
        x_act = F.tanh(self.linears_act[-1](x_act))  # tanh maps output to [-1, 1]
        # compute normal, sample, log prob, entropy
        dist_norm = torch.distributions.Normal(x_act, scale * F.softplus(self.std))
        if action is None:
